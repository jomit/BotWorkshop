...
class BasicBot {
    ...
    async onTurn(context) {
        ...
        if (context.activity.type === ActivityTypes.Message) {
            ...
            case POSTATUS_INTENT:
                //await context.sendActivity(`PO Status with entities : ${JSON.stringify(results.entities)}`);
                await this.getPOStatus(results,context);
            break;
            case NONE_INTENT:
            default:
     ...
     */
    async getPOStatus(luisResult, context){
        ...
        if (Object.keys(luisResult.entities).length !== 1) {
            ...
            await context.sendActivity(`Status for PO Number ${poNumber} is Paid.`);
        }
    }
}
